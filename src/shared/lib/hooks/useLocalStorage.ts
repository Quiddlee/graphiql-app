 ...
 * @param {string | number | boolean | Record<string | number, string | number>[]} val - The value to be stored.
 * @returns {Function} A function that retrieves the stored value from local storage.
 */
function useLocalStorage(key: string, val?: string | number | boolean | Record<string | number, string | number>[]) {
	useEffect(() => {
