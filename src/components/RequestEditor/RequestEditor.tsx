...
const RequestEditor: FC<HTMLAttributes<HTMLElement>> = ({ children, ...props }) => {
  ...
  return (
    <section
      ...
      className={cn(
        'animation-delay-200 transition-enter-screen relative flex h-full w-full origin-top animate-fade-in-section gap-4 overflow-y-hidden rounded-4xl bg-surface-container pl-2 sm:row-start-1 sm:row-end-2 sm:pl-6',
      )}
    >
      <Editor
        className="pr-20"
